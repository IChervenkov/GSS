...
const {
  ...
  presentMainPageView,
  presentWebRedirect,
} = require('./main-page.presenter');
const {
  normalizeSearchQuery,
  readCurrentCampId,
  readMainPageSessionState,
  readSessionUserId,
} = require('./http/main-page.request-context');
...
function createMainController({ useCases, env }) {
  ...
  return {
    ...
    submitUserMessage: async (req) => {
      return respondWithJson(
        await useCases.submitUserMessage({
          actorUserId: readSessionUserId(req),
          type: req.body?.type,
    ...
    adminInbox: async (req) => {
      return respondWithJson(
        await useCases.getAdminInbox({
          actorUserId: readSessionUserId(req),
          ...normalizeSearchQuery(req.query),
        }),
      );
    ...
    updateUserMessageStatus: async (req) => {
      return respondWithJson(
        await useCases.updateUserMessageStatus({
          actorUserId: readSessionUserId(req),
    ...
    deleteAdminInboxItem: async (req) => {
      return respondWithJson(
        await useCases.deleteAdminInboxItem({
          actorUserId: readSessionUserId(req),
